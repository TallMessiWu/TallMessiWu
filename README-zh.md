[![Website](https://img.shields.io/badge/🌐_tallmessiwu.com-black?style=for-the-badge)](https://tallmessiwu.com)

[![en](https://img.shields.io/badge/lang-en-blue.svg)](https://github.com/TallMessiWu/TallMessiWu/blob/main/README.md)
[![zh](https://img.shields.io/badge/lang-zh-red.svg)](https://github.com/TallMessiWu/TallMessiWu/blob/main/README-zh.md)

# 你好！👋 我是吴俊霖

> *开发者 | 摄影师 | 音乐人*

## 👨🏻‍💻 关于我

热衷于用代码构建有趣的事物。对 AI、Web 开发和数据科学充满热情。目前专注于大语言模型及其在软件工程中的应用。

- 🚀 数据科学家 & 软件工程师，坐标上海。
- 🐱 猫奴，喜欢和毛茸茸的朋友们一起度过时光。
- ⚽ 足球爱好者，空闲时间也喜欢踢球。
- 🎸 喜欢弹吉他。
- 🎹 也喜欢弹键盘。
- 📷 摄影 / 影像创作爱好者。
- ✈️ 热爱旅行，探索新的地方。
- ♟️ 国际象棋爱好者，需要放松的时候最爱下棋。
- ⌨️ DOTA 2 玩家。
- 🎮 也喜欢各种其他类型的游戏。

## 🎓 教育经历

- 🦁 **哥伦比亚大学 (Columbia University)** — 数据科学硕士（2023年8月 - 2024年12月）
- 🔱 **加州大学圣地亚哥分校 (University of California San Diego)** — 数据科学学士，辅修经济学（2019年9月 - 2023年3月）
    - 以优异成绩提前一个学期毕业，被评为优秀毕业生。
- 🏫 **德里菲尔德学校 (The Derryfield School)** — 高中毕业（2017年9月 - 2019年6月）
- 📚 **林菲尔德学校 (Linfield Christian School)** — 高中（2015年8月 - 2017年6月）

## 🏢 工作经历

- 🤖 **AI工程师** @ **华为技术有限公司**（2025年8月 - 至今）
    - 在ICT计算产品线负责模型优化与适配工作，通过分析算子profiling等数据找出模型瓶颈，并推进生态适配。
    - 同时为模型接入新加速项如融合算子等，优化模型性能。

- 🧬 **数据分析员** @ **哥伦比亚大学医学院**（2023年11月 - 2024年12月）
    - 参与活体神经元记录项目，通过分析神经元活动数据，将关键指标与动物的进食行为和进食量进行关联。
    - 编写Python脚本进行数据处理与分析，量化神经元活动与动物进食行为之间的关联，使用Matplotlib和Seaborn可视化实验数据。
    - 开发自动化数据处理流程脚本，帮助研究员实现了后续相似数据的自动化分析。

- 🎉 **开发部部长** @ **UCSD CSSA**（2020年9月 - 2023年3月）
    - 使用Vue.js、Vite和Flask开发组织网站，进行可用性测试并收集反馈以改善程序功能。组织学术研讨会和包括春节晚会在内的活动。
    - 为15名团队成员创建编程教程以提升技能。设计部门网站用于教程分享与日常信息同步。
    - [UCSD CSSA 官方网站](https://www.ucsdcssa.com) · [部门网站](https://www.wolai.com/tallmessiwu/rjeh4FU2Qrg9bXsL1SftA6)

## 💻 技术能力

`Python` `Vue.js` `TypeScript` `JavaScript` `Java` `SQL` `PyTorch` `Flask` `Docker` `Redis` `MongoDB` `Git` `Linux`

## 🚀 项目展示

| 项目 | 简介 |
| --- | --- |
| [气候变化分析](https://github.com/TallMessiWu/climate_change_data_analysis) | 使用 Python 和 D3.js 分析 100 年的气候数据，可视化全球变暖趋势。 |
| [DOTA 2 阵容助手](https://github.com/TallMessiWu/dota2-drafting-frontend) | 基于机器学习的工具，根据敌方阵容建议 DOTA 2 比赛的最佳英雄选择。 |
| [个人网站](https://tallmessiwu.com) | 一个使用 Vue 3、TypeScript 和 Element Plus 构建的现代响应式作品集网站。 |
| [UCSD CSSA 网站前端](https://github.com/TallMessiWu/ucsdcssa-website-vue) | UCSD CSSA 官方网站前端。 |
| [UCSD CSSA 网站后端](https://github.com/TallMessiWu/ucsdcssa-website-backend) | UCSD CSSA 官方网站后端服务。 |

## 📱 联系我

- 🌐 **个人网站**: [tallmessiwu.com](https://tallmessiwu.com)
- 📧 **邮箱**: Junlin-Wu@foxmail.com
- 🐙 **GitHub**: [TallMessiWu](https://github.com/TallMessiWu)
- 📸 **Instagram**: [@tallmessiwu](https://www.instagram.com/tallmessiwu/)
- 📝 **微博**: [@tallmessiwu](https://www.weibo.com/tallmessiwu/)
